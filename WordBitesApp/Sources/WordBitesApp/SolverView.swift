...
/// `wordListScrollFraction` for why `.global` instead of a named
/// coordinate space.
...
struct SolverView: View {
    ...
    // directly. An earlier attempt at this used a named coordinate space
    // on the ScrollView (geometry.frame(in: .named("wordListScroll"))) and
    // didn't work on-device. This computes the same offset a different
    // way -- the content's position minus the scroll viewport's own
    // position, both measured in plain .global coordinates -- to sidestep
    // whatever specific quirk the named-space version had, rather than
    // retrying the exact same formulation.
                ...
                if isComputing {
                    ...
                    ScrollViewReader { scrollProxy in
                        ZStack(alignment: .trailing) {
                            ...
                            // property's comment for why this attempt uses
                            // plain .global frame subtraction rather than
                            // the named-coordinate-space technique that
                            // didn't work on-device the first time.
